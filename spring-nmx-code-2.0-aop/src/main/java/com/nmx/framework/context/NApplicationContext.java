...
import com.nmx.framework.aop.NJdkDynamicAopProxy;
...
public class NApplicationContext {
    ...
    private Object instantiateBean(String beanName, NBeanDefinition beanDefinition) {
        ...
        try {
            ...
            //如果满足条件，就直接返回Proxy对象
            //1、加载AOP的配置文件
            NAdvisedSupport config = instantionAopConfig(beanDefinition);
            ...
            if(config.pointCutMath()){
                instance = new NJdkDynamicAopProxy(config).getProxy();
            }
    ...
    private NAdvisedSupport instantionAopConfig(NBeanDefinition beanDefinition) {
        NAopConfig config = new NAopConfig();
        ...
        return new NAdvisedSupport(config);
    }
